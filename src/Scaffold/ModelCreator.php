...
class ModelCreator
{
     ...
     * @return string
     */
    public function create($keyName = 'id', $timestamps = true, $softDeletes = false, $table = [])
    {
    ...
    protected function replaceFillAndCast(&$stub, $table)
    {
        $names = '';
        $casts = '';
        foreach ($table as $tb){
            $name = $tb['name'];
            $is_include = strpos($name, 'is_');
            if ($is_include !== false){
                $casts .= "'{$name}'=>'boolean', ";
            }
            $names .= "'{$name}', ";
        ...
        $stub = str_replace('DummyNames', $names, $stub);
